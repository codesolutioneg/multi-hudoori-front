...
class CustodyTypesSettingsSection extends StatefulWidget {
  ...
  @override
  State<CustodyTypesSettingsSection> createState() => _CustodyTypesSettingsSectionState();
}

class _CustodyTypesSettingsSectionState extends State<CustodyTypesSettingsSection> {
  ...
  Future<void> _load() async {
    ...
    try {
      final items = await api.custodyTypesList(activeOnly: false);
  ...
  Future<void> _openForm([Map<String, dynamic>? type]) async {
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => _CustodyTypeFormDialog(type: type),
    );
    if (ok == true) _load();
  }
  ...
  Future<void> _delete(Map<String, dynamic> type) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Text(context.t('set.custodyDelete')),
        content: Text(context.t('common.deleteQuestion', {'name': type['name']})),
        actions: [
    ...
    if (ok != true) return;
    try {
      await api.custodyTypeDelete(type['id']);
      _load();
    } catch (e) {
      _snack(e.toString());
    }
      ...
      children: [
        if (!widget.embedded) ...[
          Row(
            children: [
              Expanded(
                child: Text(context.t('set.custody'), style: TextStyle(fontWeight: FontWeight.w700)),
              ),
              IconButton(onPressed: _load, icon: Icon(Icons.refresh, size: 20)),
              FilledButton.icon(
                onPressed: () => _openForm(),
                icon: Icon(Icons.add, size: 16),
                label: Text(context.t('set.custodyNew')),
              ),
            ],
          ),
        ...
        if (_loading)
          ...
          SellixCard(
            child: Column(
              children: [
                Text(context.t('set.custodyNone')),
                const SizedBox(height: 12),
                FilledButton(onPressed: () => _openForm(), child: Text(context.t('set.custodyNew'))),
              ],
            ),
          )
        else
...
class _CustodyTypeFormDialog extends StatefulWidget {
  const _CustodyTypeFormDialog({this.type});
  final Map<String, dynamic>? type;

  @override
  State<_CustodyTypeFormDialog> createState() => _CustodyTypeFormDialogState();
}

class _CustodyTypeFormDialogState extends State<_CustodyTypeFormDialog> {
  ...
  void initState() {
    ...
    if (type != null) {
      _name.text = type['name']?.toString() ?? '';
      _sequence.text = '${type['sequence'] ?? 10}';
      _active = type['active'] != false;
    }
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (widget.type != null) {
        await api.custodyTypeUpdate(widget.type!['id'], body);
      } else {
        await api.custodyTypeCreate(body);
      }
  ...
  @override
  Widget build(BuildContext context) {
    final code = widget.type?['code']?.toString() ?? '';
    return AlertDialog(
      title: Text(widget.type == null ? context.t('set.custodyNew') : context.t('set.custodyEdit')),
      content: SizedBox(
        width: 360,
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(controller: _name, decoration: InputDecoration(labelText: context.t('set.custodyName'))),
            if (code.isNotEmpty) ...[
              const SizedBox(height: 12),
              InputDecorator(
